...
class Traversability_Map(object):
    def __init__(self, map_size, discr, Z = None, plot = False, length = 2.4, width = 2, rover_clearance = 0.5, max_inclination = 23, residual_ratio = 4, non_traversable_threshold = 245):
        self.plot = plot
        #### Map Parameters
        ...
        #### Cost Points
        self.roughness_point = 0
        self.slopes_point = 0
        self.obstacles_point = 0
        self.tot_point = 0
    ...
    def analysis(self, Z, plot = None):
        ...
        for i_x, xi in enumerate(self.x_tr):
            for i_y, yi in enumerate(self.y_tr):
                ...
                slopes_ratio = np.arccos(nz)*180/math.pi/self.max_inclination
    ...
    # Function for Traversability Analysis of a single map point
    def single_point_analysis(self, Z, xi, yi):
        if np.all(abs(xi) > abs(self.x_tr)) or np.all(abs(yi) > abs(self.y_tr)):
            self.roughness_point = None
            self.slopes_point = None
            self.obstacles_point = None
            self.tot_point = None
        else:
            # Loop to collect all points of the circle for one centre point (xi,yi)
            points_x = []
            ...
            slopes_ratio = np.arccos(nz)*180/math.pi/self.max_inclination
            sl_value = int(min([1, slopes_ratio])*255)
            self.slopes_point = sl_value        
            
            # Obstacle Test
            # Check zpoint are lower than the plane for that point + rover belly
            obstacles_ratio = errors.max()/self.rover_clearance
            ob_value = int(min([1, obstacles_ratio])*255)
            self.obstacles_point = ob_value
            
#            # Step Test
#            # Difference between maximum and minimum points in the terrain patch
#            steps_ratio = (b.max()-b.min())/rover_clearance 
#            st_value = int(min([1, steps_ratio])*255)
#            self.steps_point = st_value
            
            # Total Cost Map: if at least one of tests is over non_traversable_threshold cell is non-traversable, otherwise average
            if np.any(np.array([r_value, sl_value, ob_value]) > self.non_traversable_threshold):
                self.tot_point = 255
            else:
                self.tot_point = int(np.mean([r_value, sl_value, ob_value]))
